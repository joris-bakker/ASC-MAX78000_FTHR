...
static int reg_write(uint8_t reg, uint8_t val)
{
    ...
    int result = MXC_I2C_MasterTransaction(&i2c_req);
    MXC_Delay(8);
...
int max9867_digital_sidetone_level(headphone_mode_t mode, int level)
{
    ...
    if (level > 0)
        dvst = 0;
    else if ((mode == HPMODE_STEREO_DIFF_CLICKLESS) || (mode == HPMODE_MONO_DIFF_CLICKLESS))
        dvst = (0 - level) / 2 + 1;
    else
        dvst = ((0 - level) - 5) / 2 + 1;
...
int max9867_set_sample_rate(int sample_rate) {
    int err;
    uint16_t ni_value;

    // Wähle den NI-Wert für MCLK = 12.288 MHz basierend auf der Sample-Rate
    switch (sample_rate) {
        case 8000:   ni_value = 0x1000; break;
        case 16000:  ni_value = 0x2000; break;
        case 24000:  ni_value = 0x3000; break;
        case 32000:  ni_value = 0x4000; break;
        case 44100:  ni_value = 0x5833; break;
        case 48000:  ni_value = 0x6000; break;
        default:
            return E_INVALID; // Ungültige Sample-Rate
    }

    // Setze den PLL-Modus auf 0 (Normal-Modus)
    if ((err = reg_write(MAX9867_05_SYS_CLK, 0x00)) != E_NO_ERROR) // Deaktiviere PLL
        return err;

    // Schreibe den NI-Wert in die Register
    if ((err = reg_write(MAX9867_06_CLK_HIGH, (ni_value >> 8) & 0xFF)) != E_NO_ERROR)
        return err; // Schreibe High-Byte
    if ((err = reg_write(MAX9867_07_CLK_LOW, ni_value & 0xFF)) != E_NO_ERROR)
        return err; // Schreibe Low-Byte
